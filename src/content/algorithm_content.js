export const ALGORITHMS_CONTENT = {
    ...
    lists: [{
        ...
        header: 'хеш-таблицы (сложная структура данных)',
        items: [
            ...
            'Другие названия: ассоциативные массивы, словари, отображения, хеш-карты, хеши.',
            'Работают быстро.',
            'Во многих языках есть реализация хеш-таблиц.',
            {
                header: 'хеш-функции',
                items: [
                    ...
                    'Передаем хеш-функции слово -> она возращает число -> это число - индекс массива, где хранить данные этого слова.',
                ...
                items: [
                    ...
                    },
                    {
                        ...
                        items: [
                            ...
                            'Кэшируемые данные - хранятся в хеше.',
            ...
            },
            {
                ...
                items: [
                    ...
                    'Выбор хеш-функции действительно важен: в идеале она должна распределять ключи равноверно по всему хешу (никак не на один элемент).',
                    'Если связанный список становится слишком длинным - работа сильно замедляется. Но они не станут длинными при использовании хорошей хеш-функцииы.',
                ]
            },
            {
                ...
                items: [
                    ...
                    },
                    {
                        ...
                        items: [
                            'Кол-во элементов в хеш-таблице / Общее количество элементов.',
                            'Изменяйте размер хеш-таблицы, когда коэфф. заполнения превышает 0.7.',
            ...
            },
            {
                ...
                items: [
                    ...
                    'Направленные ациклические графы - DAG (Directed Acyclic Graph)',
                    {
                        ...
                        items: [
                            'Как выразить отношение узел А -> узел В: хеш-таблица.',
                            'Узел отображается на массив связанных с ним узлов.',
                            'Порядок добавления пары ключ-значение не важен (в хеш-таблицах элементы не упорядочены).',
                        ]
                    }
                ]
            },
        ...
        items: [
            ...
            {
                header: 'Фильтры Блума и HyperLogLog (Вероятностные структуры данных)',
                items: [
                    'Google инбексирует триллионы страниц.',
                    'Проверить проиндексирвоана ли страница - можно использовать хеш.',
                    'Но хеш получается огромным.',
                    {
                        header: 'Фильтры Блума',
                        items: [
                            'Дают ответ, который может быть ложным, но с большой вероятностью является правильным.',
                            'Возможны ложно-положительные срабатывания (фильтр скажет, что сайт обрабатывался, хотя на самом деле - нет).',
                            'Ложно-отрицательные срабатывания исключены (если фильтр говорит, что сайт не обрабатывался - можно быть в этом уверенным).',
                            'Фильтры Блума занимают мало места.',
                        ]
                    },
                    {
                        header: 'HyperLogLog',
                        items: [
                            'Действует примерно также.',
                            'Предназначен для подсчета уникальных элементов.',
                            'HyperLogLog апрокимирует количество уникальных элементов в множестве.',
                            'Не дает точного ответа, но выдает достаточно близкий результат с использованием малой части памяти.',
                        ]
                    },
                ]
            }, {
                header: 'SHA (Secure Hash Algorithm)',
                items: [
                    'Разновидность хеш-функций.',
                    'Преобразует строку не в индекс массива, а в другую строку.',
                    'Получает строку и возвращает хеш-код этой строки (уникальный для каждой).',
                    'Помогает определить, совпадают ли два файла: вычислить хеш-код файлов и сравнить.',
                    'Можно использовать для сравнения строк при отсутствии информации об исходной строке (для проверки паролей).',
                    'Является односторонним - нельзя по хешу определить исходную строку.',
                    'Золотым стандартом на 2021 год является функция bcrypt.',
                    'Локально-нечувствительный алгоритм: при изменении даже одного символа в строке, ее хеш-код полнностью изменяется (хорошо для безопасности).',
                ]
            },{
                header: 'Локально-чувствительное хеширование (Алгоритм Simhash)',
                items: [
                    'При незначительном изменении строки Simhash генерирует хеш-код, который почти не отличается от исходного.',
                    'Позволяет сравнивать хеш-коды и определять, насколько похожи две строки.',
                    'Используется для выявления сходства между фрагментами текста.',
                    'Полезно для поиска дубликтов индексирования (Google), обнаружения плагиата, поиск и обнаружение информации, запрещенной авторским правом.',
                ]
            },{
                header: 'Ключи Диффи-Хеллмана',
                items: [
                    'Решает задачу, как зашифровать сообщение, чтобы его мог прочитать только тот человек, кому адресовано сообщение.',
                    'Использует два ключа: закрытый и откытый.',
                    'Открытый известен обеим сторонам, его не нужно скрывать.',
                    'Обратная сторона зашифрует сообщение с помощью открытого ключа.',
                    'Зашифрованное сообщение можно расшифровать только с помощью закрытого ключа.',
                    'Применяется на практике вметсе с его наследником RSA.',
                ]
            },{
                header: 'Линейное программирование',
                items: [
                    'Применяется для максимизации характеристики в заданных ограничениях.',
                    'Все алгоритмы, работающие с графами, могут быть реализованы средствами линейного программирования.',
                    'Линейное программирование - намного более общая область, задачи с графами - ее подмножество.',
                    '*В лин. пр. используется симплекс-метод.',
                ]
            }
        ],
    }]
